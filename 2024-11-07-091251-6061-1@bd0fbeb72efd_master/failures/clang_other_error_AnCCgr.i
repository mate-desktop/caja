...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "eel-self-checks.c" 2
# 25 "eel-self-checks.c"
# 1 "../config.h" 1
# 26 "eel-self-checks.c" 2
...
eel_strdup_boolean (gboolean boolean)
{
    if (boolean == (0))
    {
        return g_strdup_inline ("FALSE");
    }
    if (boolean == (!(0)))
    {
        return g_strdup_inline ("TRUE");
    }
...
eel_check_string_result (char *result, const char *expected)
{
    ...
    {
        match = result != ((void*)0) && strcmp (result, expected) == 0;
    }

    if (!match)
    {
        eel_report_check_failure (result, g_strdup_inline (expected));
    }
    else
    {
        g_free (result);
