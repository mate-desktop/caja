# 1 "eel-xml-extensions.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 398 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "eel-xml-extensions.c" 2
# 25 "eel-xml-extensions.c"
# 1 "../config.h" 1
# 26 "eel-xml-extensions.c" 2
# 1 "./eel-xml-extensions.h" 1
# 28 "./eel-xml-extensions.h"
...
char *eel_strdup_vprintf_with_custom (EelPrintfHandler *custom,
                                      const char *format,
                                      va_list va);
# 29 "eel-xml-extensions.c" 2

# 1 "/usr/include/glib-2.0/glib/gi18n-lib.h" 1
# 25 "/usr/include/glib-2.0/glib/gi18n-lib.h"
                                        ...
                                        const xmlChar *property_value)
{
    ...
    if (parent == ((void*)0) || property_name == ((void*)0) || property_value == ((void*)0))
    {
        return ((void*)0);
    }
    for (child = parent->children; child != ((void*)0); child = child->next)
    {
        if (child->name != ((void*)0) && xmlStrcmp (child->name, child_name) == 0)
        {
            property = xmlGetProp (child, property_name);
            match = property != ((void*)0) && xmlStrcmp (property, property_value) == 0;
            xmlFree (property);
            if (match)
            {
                return child;
